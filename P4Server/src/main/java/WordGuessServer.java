...
// Description of File:
//
// The WordGuessServer.java file includes the main GUI application for the Server which allows the server to allow clients to connect and play
...
// The background image was resized to a dimension of 600x600 to fit the scenes

// WordGuessServer: The WordGuessServer contains the widgets, scenes, and the server which allows
// the clients to connect to the server and play the game of Word Guess
public class WordGuessServer extends Application {
	
	// Server widgets for the Port menu and the Server info screen
	...
	// PauseTransitions to display an error message to the user if success or error encountered in connection
	...
	// Variables to implement the menu bar in the port menu screen
	MenuBar optionBar;
	Menu mOne;
	MenuItem optionOne;
	// PauseTransition, revertColor, to change color of button back after a certain duration
	...
	// start():
	//
	// The start(...) method is responsible in creating the GUI application for the server side of the Word Guess game
	// in doing so, the method uses helper methods to creates Scenes and to transition the server screen
	//
	@Override
	public void start(Stage primaryStage) throws Exception {
		...
		// Define the EventHandler for revertColor to set the color of the startServer button to corn flower blue when the
		// pause is finished
		revertColor.setOnFinished(e->startServer.setStyle("-fx-background-color: cornflowerblue"));

		// Set the style and coordinates of the startServer button with setStyle(), setTranslateX() and setTranslateY() and disable the button using setDisable()
		...
		serverInfoTitle.setFont(Font.font(null, FontWeight.BOLD, 20));
		...
		numUsers.setFont(Font.font(null, FontWeight.BOLD, 20));
		...
		errorMessage.setDisable(true); // Disable errorMessage using setDisable(...)
			...
			serverConnection = new Server(data->
			{
				Platform.runLater(()->
				{
					...
					if (serverLive)
					{
						...
						// Update numUsers Text object to the current client count retrieved from getClientCount()
						numUsers.setText("Users connected: " + serverConnection.getClientCount());
						
						serverItems.add(data.toString()); // Add the retrieved data converted to a string into the ObservableList serverItems
						listItems.setItems(serverItems); // Update the ListView listItems to the serverItems contents using setItems(...)
					}
